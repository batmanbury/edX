...
class Family(object):
    ...
    def cousin(self, a, b):
        ...
        def countAncestors(name):
            anslist = []
            while name.parent != None:
                anslist.append(name.get_parent())
                name = name.get_parent()
            return anslist

        a_ans = countAncestors(self.names_to_nodes[a])
        b_ans = countAncestors(self.names_to_nodes[b])
        ...
        for n in a_ans[:-1]:
            ...
            if len(a_ans) + len(b_ans) == 0:
                c_type = -1
            elif len(a_ans) == len(b_ans):
                c_type = len(a_ans) - 1
            else:
                c_type = min(len(a_ans), len(b_ans)) - 1
                        
        degree = abs(len(a_ans) - len(b_ans))
